...
class MediaLibraryAjaxController
{
	...
	public function query( RequestInterface $request, $view, $assetType = 'all' )
	{
		...
		$mimTypes = [
			...
			'photo' => ['image/jpeg', 'image/gif', 'image/png', 'image/bmp', 'image/tiff', 'image/x-icon'],
			'video' => 'video',
			'audio' => 'audio',
			'vector' => 'image/svg+xml'
		];

		if( in_array( $assetType, array_keys( $mimTypes ) ) ){
			$queryParams['post_mime_type'] = $mimTypes[ $assetType ];
		}
