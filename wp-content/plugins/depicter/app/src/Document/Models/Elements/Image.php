...
class Image extends Models\Element
{
			...
			if ( $this->isPreviewMode || $hasRelativeUnitSize ) {
				...
				if( ! empty( $this->cropData->{$device} ) ){
					...
					if( $this->hasDataSheet() ){
						$originalMediaWidth  = wp_get_attachment_image_src( $this->renderArgs['attachmentId'], 'full' )[1] ?: $mediaWidth;
						$originalMediaHeight = wp_get_attachment_image_src( $this->renderArgs['attachmentId'], 'full' )[2] ?: $mediaHeight;
						list( $mediaWidth, $mediaHeight ) = $this->fitToSourceMedia( 'contain', $mediaWidth, $mediaHeight, $originalMediaWidth, $originalMediaHeight );
					}
	...
	protected function getImageEditOptions( $device = 'default' ){
		...
		if( $this->hasDataSheet() ){
				$originalMediaWidth  = wp_get_attachment_image_src( $this->renderArgs['attachmentId'], 'full' )[1] ?: null;
				$originalMediaHeight = wp_get_attachment_image_src( $this->renderArgs['attachmentId'], 'full' )[2] ?: null;
				list( $mediaWidth, $mediaHeight ) = $this->fitToSourceMedia( 'contain', $this->inheritedOptions['resizeW'], $this->inheritedOptions['resizeH'], $originalMediaWidth, $originalMediaHeight );
				$params[] = $mediaWidth;
				$params[] = $mediaHeight;
	...
	/**
	 * Fit image to source image with and height
	 * Mostly used in data source images
	 *
	 * @param      $type
	 * @param $sourceMediaWidth
	 * @param $sourceMediaHeight
	 * @param $width
	 * @param $height
	 *
	 * @return float[]|int[]
	 */
	protected function fitToSourceMedia( $type, $sourceMediaWidth, $sourceMediaHeight, $width, $height ) {
		$withRatio = $sourceMediaWidth / $width;
		$heightRatio = $sourceMediaHeight / $height;

		$ratio = $type === 'cover' ? max($withRatio, $heightRatio) : min($withRatio, $heightRatio);
