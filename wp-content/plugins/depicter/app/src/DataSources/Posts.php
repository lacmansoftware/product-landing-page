		...
		if ( $availableRecords && $availableRecords->have_posts() ) {
			...
			foreach( $availableRecords->posts as $post ) {
				...
				$postInfo = [
					...
					'{{{id}}}' => $post->ID,
					'{{{title}}}' => get_the_title( $post->ID ),
					'{{{featuredImageSrc}}}' => has_post_thumbnail( $post->ID ) ? wp_get_attachment_image_src( get_post_thumbnail_id( $post->ID ), 'full' )[0] : '',
					'{{{featuredImage}}}' => has_post_thumbnail( $post->ID ) ? get_post_thumbnail_id( $post->ID ) : '',
					'{{{date}}}' => get_the_date('', $post->ID ),
					'{{{excerpt}}}' => get_the_excerpt( $post->ID ),
